/*
    $Id: module-cidt.c,v 1.13 2008-01-12 16:10:21 awgn Exp $
...
MODULE_CVS("$Id: module-cidt.c,v 1.13 2008-01-12 16:10:21 awgn Exp $");
...
h_engine:       u_engine,
                ...
                token_nelm:     12,
                token_list:     { TOKEN(msec), TOKEN(idt),TOKEN(len),TOKEN(tos),TOKEN(ttl),
...
u_parser(int t, struct atom *v, cmdline_t *cmd )
{
    ...
    case TOKEN_msec:
        ...
        break;
    case TOKEN_idt:
        TAG(idt)= cast_ret(brute_eval_atom,v); 
        break;
    case TOKEN_len:
...
u_engine(cycles_t *exit_time,cmdline_t *cmd)
{
    ...
    /* print banner */
    BANNER("%s%c%d -> %s%c%d idt=%d us", 	brute_inet_ntop(p->saddr), hp_sep, p->sport,\
           brute_inet_ntop(p->daddr), hp_sep, p->dport,p->idt);
    ...
    /*** according to the idt requested, determine the interdeparture-time of packets.
      Both inter_time and HZ are expressed in time step counters (cycles_t) ***/

    inter_time = (cycles_t)(Hz/1000000*p->idt);
